...
fn snapshot_and_restore(iterations: u64, n: u64, threads: u64, value: String) {
    ...
    for k in 0..iterations {
        let store: Arc<PersistentKeyValueStore<u64, String>> = Arc::new(
            PersistentKeyValueStore::new(tmp_dir.path(), config.clone()).unwrap()
        );
            ...
            join_handles.push(
                std::thread::spawn(move || {
                    for i in 0..n {
                        ...
                        store_ref.set(i + t * n + k * n * threads, value_clone.clone());
                    }
                })
            );
        }
...
fn writes_solo(c: &mut Criterion) {
    c.bench_function("2000 random key writes from 1 thread with string keys", |b|
        b.iter(|| random_key_writes(black_box(2000), black_box(1), |i| i.to_string(), value_string(32)))
    );
    c.bench_function("2000 random key writes from 1 thread with int keys", |b|
        b.iter(|| random_key_writes(black_box(2000), black_box(1), |i| i as i32,  value_string(32)))
    );
}

fn writes_threaded(c: &mut Criterion) {
    c.bench_function("250 random key writes from 8 threads with string keys", |b|
        b.iter(|| random_key_writes(black_box(250), black_box(8), |i| i.to_string(),  value_string(32)))
    );
...
fn writes_big(c: &mut Criterion) {
    c.bench_function("250 random key writes from 8 threads with very long string keys", |b|
        b.iter(||
            random_key_writes(
                black_box(250),
                8,
...
fn snapshots(c: &mut Criterion) {
    c.bench_function("3 consecutive rounds of restoring from a snapshot", |b|
        b.iter(|| snapshot_and_restore(black_box(3), black_box(25000), black_box(8), value_string(32)))
    );
}
...
criterion_main!(writes, snapshotting);
